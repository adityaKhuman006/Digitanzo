...
$(document).ready(function(){
  ...
  $("#send").click(function(){
  	...
  	if (cur_bal < mini_bal) {
  		alert("Your current money is less than Minimum money");
  		return;
  	}
    if (cur_bal == 0) 
    {
    	...
    	$.post(
			...
			function(result){
				...
				if(result == "SUCCESS")
					...
					alert("Your request was not sent to admin successfully!!! Please wait until when admin allow your withdrawal.");
						...
						if (res[0]["yes"] == "yes") 
						{
							if (requested_count < count) 
							{
								...
								for (var i = 0; i < count; i++) 
								{
									...
									{									
										var notifyTxt = " Congratulation!. You withdrawal of " + res[i]["payed"] + " at " + res[i]["date"] + " was successed by allow of admin!"	
										noty({ 
						    				   text: notifyTxt,
											   closeWith: ['click'],
											   killer: false,
			...
			function(result){
				...
				for (var i = 0; i < count; i++) 
				{
					...
					{	
						...
						var notifyTxt = " Sorry!. Your withdrawal of " + res[i]["payed"] + " at " + res[i]["date"] + " was declined by admin!"	
						noty({ 
								text: notifyTxt,
								closeWith: ['click'],
								killer: false,
