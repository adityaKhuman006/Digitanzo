...
$me_user_reg_id=$get_data['user_reg_id'];
$me_user_rec_id=$get_data['user_rec_id'];
//echo $user_reg_id;
//die;
                ...
                <table class="table table-bordered table-striped">
                      ...
                      <tbody>
                        <?php
                          ...
                          SELECT @pv:=(SELECT GROUP_CONCAT(user_rec_id SEPARATOR ',') FROM tb_user WHERE user_rec_id IN (@pv)) AS ga FROM tb_user
                          JOIN
                          (SELECT @pv:=$me_user_rec_id)tmp
                          WHERE user_rec_id IN (@pv)) a");
                          ...
                          $check_users=$db->prepare("SELECT user_no,user_reg_id,user_name,user_rec_id,user_dir,regist_date FROM
                          (SELECT user_no,user_reg_id,user_name,user_rec_id,user_dir,regist_date,
                          ...
                          $check_users->bind_result($user_no,$user_reg_id,$user_name,$user_rec_id,$user_dir,$regist_date);
                            $user_data=[];
                            $count=1;
                            while($check_users->fetch()){
                                $tmp=[];
                                if($count<=3){
                                  if($me_user_rec_id==$user_reg_id){
                                    $tmp['user_add']=$count;
                                ...
                                /*if($user_dir>0){
                                  for($i=1;$i<=$user_dir;$i++){
                                    $tmp1=[];
                                    $tmp1['user_add']=$count;
                                    $tmp1['user_name']=$user_name;
                                    $tmp1['commission']=333;
                                    $tmp1['add_date']=$regist_date;
                                    $tmp1['user_rec_id']=$user_rec_id;
                                    array_push($user_data, $tmp1);
                                  }
                                }*/
